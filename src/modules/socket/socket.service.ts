...
export class SocketService {
  ...
  async handleAGUIEvent(payload: AGUIEvent, server: Server) {
    console.log(
      `[${payload.threadId} - ${payload.sessionId} - ${payload.event.messageId} -> ${payload.event.type}] ${payload.event.delta || ''}`,
    );
    ...
    try {
      ...
      await redis.lpush(
        `thread:${payload.threadId}:${payload.sessionId}`,
      ...
      switch (payload.event.type) {
        case EventType.RUN_STARTED:
          ...
          await this.threadService.updateThread(payload.threadId, {
        ...
        case EventType.RUN_FINISHED:
          ...
          await Promise.all([
            ...
            this.threadService.updateThread(payload.threadId, {
        ...
        case EventType.TEXT_MESSAGE_START:
          ...
          await this.threadService.createEmptyAgentMessage(
            payload.threadId,
            payload.sessionId,
